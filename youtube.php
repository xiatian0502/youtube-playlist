...
function fetchWithRetry($url, $apiKeys, &$apiKeyIndex, $maxRetries = 5, $delay = 2) {
    $retryCount = 0;
...
foreach ($playlistIds as $playlistId) {
    ...
    // 记录API请求和响应
    file_put_contents($log, "API Request: {$apiUrl}&key=\nResponse: " . print_r($videoList, true) . "\n", FILE_APPEND);
    ...
    foreach ($videoList['items'] as $item) {
        ...
        // 记录频道请求和响应
        file_put_contents($log, "Channel Request: {$channelUrl}&key=\nResponse: " . print_r($channelInfo, true) . "\n", FILE_APPEND);
